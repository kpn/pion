...
func (verifier AWSV4Verifier) checkAdminRequestAuthType(ctx context.Context, r *http.Request, region string) APIErrorCode {
	...
	if _, ok := r.Header["X-Amz-Content-Sha256"]; ok &&
		...
		s3Err = verifier.isReqAuthenticated(ctx, r, region)
	}
	if s3Err != ErrNone {
...
func (verifier AWSV4Verifier) checkRequestAuthType(ctx context.Context, r *http.Request, action policy.Action, bucketName, objectName string) (s3Err APIErrorCode) {
...
func reqSignatureV4Verify(r *http.Request, region string) (s3Error APIErrorCode) {
	...
	return verifier.reqSignatureV4Verify(r, region)
}

func (verifier AWSV4Verifier) reqSignatureV4Verify(r *http.Request, region string) (s3Error APIErrorCode) {
